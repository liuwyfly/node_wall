...
(async () => {
    ...
    await page.waitFor(2000);
    await page.screenshot({path:"/usr/local/nginx/html/tf01.png", clip : {x:0, y:0, width:1366, height:documentSize.height}});
    const content = await page.content();

    // console.log(content);
    fs.writeFile('/usr/local/nginx/html/tf01.txt', content, function(err){
